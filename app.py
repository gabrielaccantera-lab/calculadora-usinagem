...
def gerar_tabelona_pura(resultados, tempo, dist, aplic, pmp, dias, horas_turno, horas_efetivas, thresholds):
    ...
    for mes_t in MESES:
        ...
        # Linha 4: volumes PMP por modelo (igual ao Excel de referência)
        for _mi4, _mod4 in enumerate(modelos_lista):
            _q4 = _qtd_mes.get(_mod4, 0)
            _ec(ws_out, 4, 20+_mi4, _q4 if _q4 else None, _F_AMAR, True, "000000", 8)
        ri_t=7
    ...
    if file_bytes is not None:
        try:
            ...
            if "AnoFY26" in wb_ref.sheetnames:
                ...
                result = []
                # Detecta formato pelo cabeçalho (linha 6, índice 5)
                _hdr = [c.value for c in rows_ref[5]] if len(rows_ref) > 5 else []
                _has_seq  = any(str(v or "").strip().upper() in ("SEQ","#") for v in _hdr[:3])
                _has_perf = any(str(v or "").lower().startswith("perf") or str(v or "").lower().startswith("performance") for v in _hdr)
                _off_ano  = 1 if _has_seq else 0
                _is_ano   = 1 if _has_perf else 0
                for row in rows_ref[6:]:
                    vals = [c.value for c in row[:21]]
                    if not str(vals[_off_ano] or "").startswith("CEN"): continue
                    try:
                        cen  = str(vals[_off_ano]); peca = str(vals[_off_ano+1])
                        pc_t = float(vals[_off_ano+3] or 1)
                        tc   = float(vals[_off_ano+5] or 0); tl = float(vals[_off_ano+6] or 0)
                        dc   = float(vals[_off_ano+7] or 0); vi = float(vals[_off_ano+8] or 1)
                        dv   = float(vals[_off_ano+9] or 0); di = float(vals[_off_ano+10] or 1)
                        _po  = float(vals[_off_ano+11] or 1) if _is_ano else 1.0
                        idx  = float(vals[_off_ano+11+_is_ano] or 0)
                        mc   = float(vals[_off_ano+14+_is_ano] or 0); ml = float(vals[_off_ano+15+_is_ano] or 0)
                        qt   = int(vals[_off_ano+16+_is_ano] or 0)
                        result.append((cen, peca, pc_t, tc, tl, dc, vi, dv, di, _po, idx, mc, ml, qt))
                    except: pass
                wb_ref.close()
...
def gerar_aba_anual(wb, resultados, label="ANO", cp_data=None, horas_anual=None, eh_cenario=False, ws_existente=None, res_ano_override=None, pmp=None, aplic=None):
    ...
    modelos_lista_a = sorted(pmp["modelo"].unique().tolist()) if pmp is not None else []
    n_mod_a = len(modelos_lista_a)
    _aplic_set_a = set(zip(aplic["centro"], aplic["peca"], aplic["modelo"])) if aplic is not None else set()
    _qtd_anual_a = {mod: int(pmp[pmp["modelo"]==mod]["qtd"].sum()) for mod in modelos_lista_a} if pmp is not None else {}
    ...
    # Linha 4: volumes PMP anuais por modelo
    for _mi4a,_mod4a in enumerate(modelos_lista_a):
        _q4a=_qtd_anual_a.get(_mod4a,0); _e(ws,4,20+_mi4a,_q4a if _q4a else None,PatternFill("solid",fgColor="FFDE00"),True,"000000",8)
    ws.row_dimensions[4].height=13
    # LINHA 5 — 19 colunas fixas + colunas de modelos
    ws.merge_cells(f"A5:{get_column_letter(19+n_mod_a)}5"); _e(ws,5,1,f"RESUMO DA CARGA — ANO ({dias_ano} dias / {n_meses} meses)",F_VD_JD_a,True,"FFFFFF",10,True)
    ...
    for mi_a,mod_a in enumerate(modelos_lista_a):
        ci_a=20+mi_a; _e(ws,6,ci_a,mod_a,F_CINZA_a,True,"000000",7,True,True); ws.column_dimensions[get_column_letter(ci_a)].width=7
    ...
    if cp_data:
        ...
        for (cen,peca,_pc_t,tc,tl,dc,vi,dv,di,po,idx_c,mc,ml,_qtd) in cp_data:
            ...
            for mi_a2,mod_a2 in enumerate(modelos_lista_a):
                ci_a2=20+mi_a2; v_a=_qtd_anual_a.get(mod_a2,0) if (cen,peca,mod_a2) in _aplic_set_a else 0
            ...
            with st.spinner("Gerando tabelona... (~10s)"):
                ...
                try:
                    ...
                    if _aba_ref_t is None:
                        ...
                        # Detecta colunas de modelo: suporta formato antigo (linha 6 = "MODELO X")
                        # e formato novo (linha 5 = "MODELO X", linha 6 = nome real como "5060E OOS")
                        modelos_xl_t = []; modelo_col_idx = {}
                        for _mc in range(19, 88):
                            _v5 = ws_nov_t.cell(5, _mc).value
                            _v6 = ws_nov_t.cell(6, _mc).value
                            if _v5 and str(_v5).startswith("MODELO"):
                                _mname = str(_v6) if _v6 else str(_v5)
                                modelos_xl_t.append(_mname); modelo_col_idx[_mname] = _mc - 19
                            elif _v6 and str(_v6).startswith("MODELO"):
                                modelos_xl_t.append(str(_v6)); modelo_col_idx[str(_v6)] = _mc - 19
                        ...
                        for mes_t in MESES:
                            ...
                            # Linha 4: volumes PMP por modelo (igual ao Excel de referência)
                            for _mi4,_mod4 in enumerate(modelos_xl_t):
                                _q4=int(pmp_mes_t[pmp_mes_t.modelo==_mod4]["qtd"].sum()) if _mod4 in pmp_mes_t.modelo.values else 0
                                _ec(ws_out,4,20+_mi4,_q4 if _q4 else None,_F_AMAR,True,"000000",8)
                            ws_out.row_dimensions[4].height=13
                            ...
                            # Formato novo: SEQ em col A (+1) e perf_op entre disponib/indice (+1 extra)
                            # base_row_t[0]=SEQ, [1]=MÁQUINA, [2]=REFERÊNCIA, [3]=DESCRIÇÃO
                            # [4]=PEÇA TRAT, [5]=UM, [6]=t_ciclo, [7]=t_labor
                            # [8]=div_carga, [9]=vol_int, [10]=div_volume, [11]=disponib
                            # [12]=perf_op(novo), [13]=indice_ciclo, [14]=JA.A, [15]=JA.B
                            # [16]=JA.C, [17]=TOTAL CICLOS, [18]=TOTAL LABOR, [19]=TOTAL PECAS(se existir)
                            _aplic_set_tab = set(zip(aplic["centro"], aplic["peca"], aplic["modelo"]))
                            main_data_t=dm_t.get("main",[]); vols_data_t=dm_t.get("vols",[])
                            for ri_t_idx,base_row_t in enumerate(base_rows_t):
                                cen_t=str(base_row_t[1]).strip(); peca_t=str(base_row_t[2]).strip()
                                ri_t=7+ri_t_idx
                                tc_xl_t=base_row_t[6]; tl_xl_t=base_row_t[7]
                                dc_xl_t=base_row_t[8]; vi_xl_t=base_row_t[9]; dv_xl_t=base_row_t[10]
                                di_xl_t=base_row_t[11]; idx_xl_t=base_row_t[13]

                                mrow_t=main_data_t[ri_t_idx] if ri_t_idx<len(main_data_t) else [None]*21
                                xl_pA_t=mrow_t[14] if len(mrow_t)>14 else None
                                xl_pB_t=mrow_t[15] if len(mrow_t)>15 else None
                                xl_ciclo_t=mrow_t[17] if len(mrow_t)>17 else None
                                xl_pecas_t=mrow_t[19] if len(mrow_t)>19 else None
                                # Lê os valores de distribuição do MÊS CORRENTE (não da aba estrutural)
                                # para comparar corretamente com o INPUT
                                dc_xl_t = mrow_t[8]  if len(mrow_t)>8  else base_row_t[8]
                                vi_xl_t = mrow_t[9]  if len(mrow_t)>9  else base_row_t[9]
                                dv_xl_t = mrow_t[10] if len(mrow_t)>10 else base_row_t[10]
                                di_xl_t = mrow_t[11] if len(mrow_t)>11 else base_row_t[11]
                                idx_xl_t = mrow_t[13] if len(mrow_t)>13 else base_row_t[13]
                                ...
                                for mod_t2 in modelos_xl_t:
                                    ...
                                    flag_t = 1 if (cen_t, peca_t, mod_t2) in _aplic_set_tab else 0
                                ...
                                pc_trat_t = (float(_tr_t.pc_trat) if _tr_t is not None and hasattr(_tr_t,"pc_trat") else None) or (float(base_row_t[4]) if base_row_t[4] else 1.0)
                                ...
                                _ec(ws_out,ri_t,3,base_row_t[3],_F_BRANCO,False,"000000",8,False)
                                _ec(ws_out,ri_t,4,base_row_t[4],_F_BRANCO,False,"000000",8)
                                _ec(ws_out,ri_t,5,base_row_t[5],_F_BRANCO,False,"000000",8)
